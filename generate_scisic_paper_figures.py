#!/usr/bin/env python3
"""
Generate SCISIC paper figures aligned with the TBCR figure set names.
...
Notes:
  - Uses SCISIC stage1 robustness artifacts from eval_v2.
  - Distill/distill_v2 artifacts are currently unavailable for SCISIC,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate SCISIC paper figures")
    p.add_argument(
        "--runs-root",
        default="runs/classify/train_scisic_final_eval_v2/scisic",
        help="Root containing <variant>_small/stage1/robustness/robustness_results.json",
    )
    p.add_argument(
        "--dataset-root",
        default="/data2/enoch/ekd_coding_env/meddef_winlab/processed_data/scisic",
        help="SCISIC processed dataset root with val/test splits",
    )
    p.add_argument("--split", default="val", choices=["val", "test", "train"])
    p.add_argument("--out", default="out/scisic_figures")
    return p.parse_args()
...
def fig_class_distribution(dataset_root: Path, split: str, out_dir: Path) -> None:
    ...
    # Use consistent palette based on class count
    if len(class_names) <= 4:
        # For SCISIC (typically 4 classes), use a curated set
        color_palette = ["#1565C0", "#2E7D32", "#E65100", "#6A1B9A"]
    ...
    ax.set_xlabel("Class")
    ax.set_ylabel("Count")
    ax.set_title(f"SCISIC {split.capitalize()} Split Class Distribution")
...
def fig_radar(metrics: Dict[str, Dict[str, float]], out_dir: Path) -> None:
    # Metrics chosen from available stage1 SCISIC artifacts.
    metric_defs = {
...
def fig_stage_improvement(metrics: Dict[str, Dict[str, float]], out_dir: Path) -> None:
    # SCISIC: stage1 is complete; distill/distill_v2 artifacts are not available yet.
    ...
    fig.text(
        0.5,
        0.01,
        "SCISIC distill/distill_v2 training artifacts are not yet available; figure shows stage1 comparisons.",
        ha="center",
        fontsize=8,
        color="dimgray",
    )
...
def main() -> None:
    ...
    print("Generating SCISIC figures...")
