#!/usr/bin/env bash
################################################################################
# MedDef2 — CHEST_XRAY FINAL Training Run
...
#
# Usage:
#   bash run/train_chest_xray_final.sh                  # start / resume all
#   bash run/train_chest_xray_final.sh --status         # check status
#   bash run/train_chest_xray_final.sh --watch          # live dashboard
#   bash run/train_chest_xray_final.sh --live           # tail all training logs
#   bash run/train_chest_xray_final.sh --stop           # stop the scheduler
#   bash run/train_chest_xray_final.sh --clean          # wipe state and restart
#   bash run/train_chest_xray_final.sh --install-cron   # auto-start on reboot
#   bash run/train_chest_xray_final.sh --remove-cron    # remove reboot hook
#   bash run/train_chest_xray_final.sh --dry-run        # simulate (no actual training)
#
# Override any param via env:
#   GPU_IDS=2,3 bash run/train_chest_xray_final.sh
#   EPOCHS=200  bash run/train_chest_xray_final.sh
################################################################################
...
RUN_NAME="${RUN_NAME:-train_chest_xray_final}"  # output → runs/classify/train_chest_xray_final/chest_xray/<variant>/
...
# DIRECTORIES
# ─────────────────────────────────────────────────────────────────────────────
LOG_BASE="$PROJECT_DIR/logs/chest_xray_final"
...
last_pt_for_variant() {
  local variant=$1
  echo "$PROJECT_DIR/runs/classify/${RUN_NAME}/chest_xray/${variant}_${DEPTH}/weights/last.pt"
}

best_pt_for_variant() {
  local variant=$1
  echo "$PROJECT_DIR/runs/classify/${RUN_NAME}/chest_xray/${variant}_${DEPTH}/weights/best.pt"
}
...
variant_is_done() {
  ...
  local results; results="$PROJECT_DIR/runs/classify/${RUN_NAME}/chest_xray/${variant}_${DEPTH}/results.csv"
...
launch_variant() {
  ...
  local cmd=(
    ...
    --project "runs/classify/${RUN_NAME}/chest_xray"
...
show_dashboard() {
  ...
  echo ""
  echo -e "${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
  printf  "${CYAN}║${NC}  ${WHITE}${BOLD}MedDef2 CHEST_XRAY FINAL Run${NC}  %-37s${CYAN}║${NC}\n" "Runtime: $(printf '%02dh%02dm%02ds' $h $m $s)"
  echo -e "${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
  echo ""
...
cmd_status() {
  ...
  echo ""
  echo -e "${BOLD}${BLUE}╔══════════════════════════════════════════════════╗${NC}"
  echo -e "${BOLD}${BLUE}║     MedDef2 CHEST_XRAY FINAL — STATUS                  ║${NC}"
  echo -e "${BOLD}${BLUE}╚══════════════════════════════════════════════════╝${NC}"
  echo -e "  Run tag  : ${WHITE}${RUN_NAME}${NC}"
  echo -e "  Output   : runs/classify/${RUN_NAME}/chest_xray/<variant>_${DEPTH}/"
...
# ─────────────────────────────────────────────────────────────────────────────
cmd_live() {
  echo -e "${CYAN}${BOLD}== LIVE TRAINING LOGS — CHEST_XRAY FINAL ==${NC}"
...
run_scheduler() {
  ...
  log_info "════════════════════════════════════════════════════════"
  log_info "MedDef2 CHEST_XRAY FINAL — Scheduler started (PID $$)"
  log_info "Run tag    : ${RUN_NAME}"
  log_info "Dataset    : ${CHEST_XRAY_DATA}"
...
# This means 'bash train_chest_xray_final.sh' always returns the prompt immediately;
# training can never be killed by Ctrl+C in any terminal.
if [[ -z "${_CHEST_XRAY_DAEMON:-}" && "${_DRY_RUN:-0}" -ne 1 ]]; then
  mkdir -p "$LOG_BASE"
  export _CHEST_XRAY_DAEMON=1
...
# Verify data exists
if [[ ! -d "$CHEST_XRAY_DATA" ]] && [[ "${_DRY_RUN:-0}" -ne 1 ]]; then
  log_err "CHEST_XRAY dataset not found at: $CHEST_XRAY_DATA"
  log_err "Set CHEST_XRAY_DATA=/path/to/dataset and re-run"
  exit 1
fi
