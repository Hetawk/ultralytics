#!/usr/bin/env bash
################################################################################
# MedDef2 — SCISIC FINAL Stage-2 Defensive Distillation  [v2: T=50, α=0.9]
#
...
#   runs/classify/train_scisic_final/scisic/<variant>_small/distill_v2/
#
# Features:
...
#
# Usage:
#   bash run/distill_scisic_final.sh
#   bash run/distill_scisic_final.sh --status
#   bash run/distill_scisic_final.sh --watch
#   bash run/distill_scisic_final.sh --stop
#   bash run/distill_scisic_final.sh --dry-run
################################################################################
...
LOG_BASE="$PROJECT_DIR/logs/scisic_final_${DISTILL_NAME}"
...
distill_dir_for_variant() {
  local variant=$1
  echo "$PROJECT_DIR/runs/classify/${RUN_NAME}/scisic/${variant}_${DEPTH}/${DISTILL_NAME}"
}
...
launch_variant() {
  ...
  local variant_dir="runs/classify/${RUN_NAME}/scisic/${variant}_${DEPTH}"
...
show_status() {
  ...
  echo
  echo "Stage-2 SCISIC distillation status"
...
if [[ "${MEDDEF_DISTILL_DAEMON:-0}" != "1" && "${_DRY_RUN:-0}" != "1" ]]; then
  ...
  echo "[INFO] Check status with: bash run/distill_scisic_final.sh --status"
  exit 0
fi
