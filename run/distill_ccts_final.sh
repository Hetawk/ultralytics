#!/usr/bin/env bash
################################################################################
# MedDef2 — CCTS FINAL Stage-2 Defensive Distillation  [T=50, α=0.9]
# Mirrors distill_tbcr_final.sh for the CCTS dataset.
#
# Reuses the CCTS final stage-1 checkpoints as both:
#   1) student initialisation (`--pretrained`)
#   2) frozen teacher checkpoint (`--teacher_model`)
#
# Output layout for each variant:
#   runs/classify/train_ccts_final/ccts/<variant>_small/distill_v2/
#
# Features:
...
#
# Usage:
#   bash run/distill_ccts_final.sh
#   bash run/distill_ccts_final.sh --status
#   bash run/distill_ccts_final.sh --watch
#   bash run/distill_ccts_final.sh --stop
#   bash run/distill_ccts_final.sh --dry-run
################################################################################
...
# Distillation hyperparameters — matched to TBCR for comparability.
# 50 epochs (vs TBCR's 100) is proportional to CCTS dataset size (~5x smaller).
EPOCHS="${EPOCHS:-50}"
BATCH="${BATCH:-32}"
IMGSZ="${IMGSZ:-224}"
DEPTH="${DEPTH:-small}"
...
# Disable early stopping: let all variants train for full EPOCHS.
PATIENCE="${PATIENCE:-0}"
DROPOUT="${DROPOUT:-0.0}"
ERASING="${ERASING:-0.2}"
MIXUP="${MIXUP:-0.0}"
...
# Defensive distillation temperature and alpha (same as TBCR for comparability)
...
LOG_BASE="$PROJECT_DIR/logs/ccts_final_${DISTILL_NAME}"
...
distill_dir_for_variant() {
  local variant=$1
  echo "$PROJECT_DIR/runs/classify/${RUN_NAME}/ccts/${variant}_${DEPTH}/${DISTILL_NAME}"
}
...
launch_variant() {
  ...
  local variant_dir="runs/classify/${RUN_NAME}/ccts/${variant}_${DEPTH}"
  ...
  for gpu in "${GPU_ARR[@]}"; do
    ...
    if [[ "$status" == "completed" ]]; then
      if [[ "${_DRY_RUN:-0}" -eq 1 ]] || variant_is_done "$variant"; then
        ...
        log_info "GPU $gpu: $variant exited cleanly at epoch $((progress_epoch + 1))/$EPOCHS; re-queuing"
...
show_status() {
  ...
  echo
  echo "Stage-2 CCTS distillation status"
...
if [[ "${MEDDEF_CCTS_DISTILL_DAEMON:-0}" != "1" && "${_DRY_RUN:-0}" != "1" ]]; then
  echo "[INFO] Starting CCTS distillation scheduler in background..."
  nohup env MEDDEF_CCTS_DISTILL_DAEMON=1 bash "$SCRIPT_PATH" "$@" >> "$LOG_BASE/nohup.out" 2>&1 &
  echo $! > "$PID_FILE"
  echo "[INFO] Scheduler PID: $(cat "$PID_FILE")"
  echo "[INFO] Check status with: bash run/distill_ccts_final.sh --status"
  exit 0
fi

echo $$ > "$PID_FILE"
queue_remaining
log_info "Starting CCTS stage-2 distillation scheduler"
log_info "Dataset : CCTS (4-class CT, 1000 imgs)  T=${DIST_TEMP}  α=${DIST_ALPHA}"
log_info "Epochs  : ${EPOCHS}  Batch: ${BATCH}  Patience: ${PATIENCE} (disabled)"
...
while true; do
  ...
  if [[ ! -s "$QUEUE_FILE" && "$any_running" -eq 0 ]]; then
    log_ok "All eligible CCTS distillation jobs completed."
    break
  fi
