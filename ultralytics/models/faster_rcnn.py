...
class YoloDetectionDataset(Dataset):
        ...
        if label_path.exists():
            with label_path.open() as lf:
                for line in lf:
                    ...
                    coords = self._yolo_to_pascal(cx, cy, bw, bh, width, height)
                    if coords is None:
                        continue
                    cls_idx = int(cls) + 1  # TorchVision detectors reserve 0 for background
    ...
    @staticmethod
    def _yolo_to_pascal(cx: float, cy: float, bw: float, bh: float, width: int, height: int) -> list[float] | None:
        x_c = cx * width
        y_c = cy * height
        half_w = (bw * width) / 2.0
